...
public class ExhibitorManagerService extends ExhibitorService {
    ...
    public QueryExhibitorResponse queryExhibitorsByPage(QueryExhibitorRequest request) throws UnsupportedEncodingException {
        ...
        if (StringUtils.isNotEmpty(request.getCompany())) {
        	conditions.add(" (e.company like '%" + new String(request.getCompany().getBytes("ISO-8859-1"),"GBK") + "%' OR e.company like '%" + new String(request.getCompany().getBytes("ISO-8859-1"),"utf-8") + "%') ");
        }
        if (StringUtils.isNotEmpty(request.getCompanye())) {
            conditions.add(" (e.companye like '%" + new String(request.getCompanye().getBytes("ISO-8859-1"),"GBK") + "%' OR e.companye like '%" + new String(request.getCompanye().getBytes("ISO-8859-1"),"utf-8") + "%') ");
        }
        ...
        List<QueryExhibitor> exhibitors = exhibitorDao.queryPageByHQL("select count(*) from TExhibitor e " + conditionsSqlNoOrder,
        		"select new com.zhenhappy.ems.manager.dto.QueryExhibitor(e.eid, e.username, e.password, e.area, e.company, e.companye, e.country, e.province, e.isLogout, e.tag, e.group, b.boothNumber, e.exhibitionArea) "
        ...
        for(QueryExhibitor exhibitor:exhibitors){
    		...
    		if(exhibitorInfo != null){
    			if(StringUtils.isNotEmpty(exhibitorInfo.getEmail()) || StringUtils.isNotEmpty(exhibitorInfo.getPhone())) exhibitor.setInfoFlag(1);
